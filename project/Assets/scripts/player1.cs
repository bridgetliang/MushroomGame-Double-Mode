...
public class player1 : MonoBehaviour
{
    ...
    {
        if (PlayerManager1._instance.isDefeat1)  //如果被打败
        {
            return;
        }
    ...
    void Move()
    {
        float h = Input.GetAxisRaw("player1Horizontal");//水平轴，取值范围x方向（-1，1）
    ...
    void Attack()
    {
        if (Input.GetKeyDown(KeyCode.Tab))  //GatKeyDown(获取某个键的按下)
        {
    ...
    void Die()
    {
        ...
        PlayerManager1._instance.isDead1 = true;//利用单利模式进行数据访问
